...
try:
    # Reuse the existing code-pinned token without duplicating it in V2 files.
...
def load_config() -> WorkerConfig:
    state_home = expand(os.environ.get("UTAT_V2_STATE_HOME", "~/.utat-node-v2"))
    return WorkerConfig(
        node_id=os.environ.get("UTAT_V2_NODE_ID", "local"),
        state_home=state_home,
        db_path=expand(os.environ.get("UTAT_V2_DB", str(state_home / "queue.db"))),
        ...
        poll_interval_sec=float(os.environ.get("UTAT_V2_POLL_INTERVAL", "5")),
        idle_exit_sec=float(os.environ.get("UTAT_V2_IDLE_EXIT", "300")),
        web_host=os.environ.get("UTAT_V2_WEB_HOST", "0.0.0.0"),
        web_port=int(os.environ.get("UTAT_V2_WEB_PORT", "8766")),
    )
