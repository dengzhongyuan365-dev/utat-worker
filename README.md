...
安装脚本用于 `AT/UT-研发自测` 流程。脚本只安装 AT/UT worker，不安装、不登录、不配置 `multica`；运行环境应提前具备可用的 `multica` CLI。
...
- 环境变量写入 `~/.config/utat-worker/env`，并在 `~/.bashrc` 增加受控 block，打开新 shell 后永久生效；
- 命令入口写入 `~/.local/bin`。
...
```bash
utat-worker status
utat-worker submit --payload-file payload.json
utat-worker submit --payload-file payload.json --rerun
utat-worker-daemon
utat-worker-web
```
...
- 停止本机 worker/web 进程；
- 删除 `~/.local/bin/utat-worker*` 命令；
- 删除 `~/.bashrc` 中的环境变量 block；
- 删除 `~/.utat-worker`、`~/.config/utat-worker`；
- 删除 `~/.utat-node`、`~/atut-work`、`~/Documents/ATUT-WORK-Archive`；
- 删除 `~/WorkSpace/utat-worker`。

如果想保留源码或数据：

```bash
KEEP_SOURCE=1 KEEP_DATA=1 bash -c "$(curl -fsSL https://raw.githubusercontent.com/dengzhongyuan365-dev/utat-worker/master/scripts/uninstall-worker.sh)"
```
