...
- 同一台执行机同时最多只跑一个 AT/UT；
- 多台执行机之间允许并发，每台机器各自保持本地串行；
- 一个应用跑完 AT 再跑 UT；
- 不同应用可以被分配到不同机器并行；
- 多个入口 issue 同时启动时也必须受节点本地锁限制，不能把机器打爆。
...
```text
稳定跑通总入口 issue 的完整 AT/UT 流程；同一节点串行，多节点并行。
```
...
确保同一台执行机同一时间只跑一个 AT/UT。不同执行机拥有不同 `node_id`，因此可以并发。
...
中心调度器不是 Multica Agent，也不默认运行在用户个人电脑上。它需要部署在一台稳定、长期在线、所有 worker 都可以访问的公共控制节点或内网服务上。
...
70、test 等机器只运行 worker。真实中心节点地址、节点名称和路由都通过配置注入，不写死在代码里。

如果不存在所有 worker 都能访问的公共控制节点，则多机器调度不能直接上线；需要先提供一个内网服务地址，或选择具备网络可达性的管理节点。
...
示例：local 跑 deepin-mail AT 的同时，public-test 跑 deepin-editor AT 是允许的；local 上不会再领取第二个 AT/UT。
